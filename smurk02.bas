 ...
 6040  PRINT #1, L: FOR B = 1 TO NI: PRINT #1, IL%(B): NEXT B: PRINT #1, T
 6050  PRINT #1, SC%: PRINT #1, SM%: PRINT #1, T%
 6060  FOR B = 1 TO NI: PRINT #1, SC%(B): NEXT B
 6070  PRINT #1, LO%: PRINT #1, MX%
 6080  PRINT #1, PN%
 6090  REM  PRINT D$"CLOSE"
 6091  CLOSE #1
 ...
 6530  REM  PRINT D$"OPEN GAME."NA$",L500"
 6531  I$=NA$+CHR$(49+A)+".DAT": OPEN I$ FOR INPUT AS #1
 6533  REM  PRINT D$"OPEN GAME."NA$",L500"
 6540  REM  PRINT D$"READ GAME."NA$",R"A
 6550  INPUT #1, L: FOR B = 1 TO NI: INPUT #1, IL%(B): NEXT B: INPUT #1, T
 6560  INPUT #1, SC%: INPUT #1, SM%: INPUT #1, T%
 6565  FOR B = 1 TO NI: INPUT #1, SC%(B): NEXT B
 6570  INPUT #1, LO%: INPUT #1, MX%
 6580  INPUT #1, PN%
 6590  REM  PRINT D$"CLOSE"
 6591  CLOSE #1
 6595  REM RH POKE 216,0  (Cancel the ON ERR function)
 6600  RETURN 
 6900  REM  ONERR
